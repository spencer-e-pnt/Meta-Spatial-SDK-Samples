...
fun QuizButton(
  ...
  val backgroundColor = if(hasSelectedAnswer) {
    if(isCorrectAnswer) {
  ...
  val regularButtonModifier = Modifier.height(dimensionResource(com.meta.spatial.uiset.R.dimen.button_height))
  val buttonModifier = if(hasSelectedAnswer && didAnswerCorrectly && !isCorrectAnswer) {
    regularButtonModifier.then(Modifier.alpha(0.35f))
  } else regularButtonModifier

  return Box(
    modifier = Modifier
      .scaleOnPressed(interactionSource = interactionSource)
      .then(buttonModifier)
